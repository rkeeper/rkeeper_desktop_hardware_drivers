//При отключенном XML_WIDE_CHARS перекодировка в utf-8 и назад снаружи, если не использовать SetVarAttr и GetVarAttr.
//Если использовать SetVarAttr и GetVarAttr, то надо выставлять LoadUTF8AsWideString=true (тогда внутри строки будут храниться раскодированными в юникоде, а GetAttr будет перекодировать по дефолтовой локале)
//UTF-16 поддерживается только XML_WIDE_CHARS и только UTF-16LE
...
begin
	aPos := 1;
	y := FetchTo('-'); m := FetchTo('-');
  if y = 0 then // Если только время, без даты
    aPos := 1;
  d := FetchTo('T');
...
begin
	...
	//'=': Result := 0; - сюда = уже не должно попасть, см. CharsToOctet
...
function TXmlNameTable.GetID(const aName: TXmlString; AnyCaseFirst:boolean): Integer;
  ...
  begin
    ...
    if Length(Result) > 0 then begin
      if (Result[1] >= 'A') and (Result[1] <= 'Z') then
        Result[1] := char(Ord(Result[1]) + 32); // делаем из большой буквы маленькую
    end;
  end;
  ...
  begin
    ...
    if Length(Result) > 0 then begin
      if (Result[1] >= 'a') and (Result[1] <= 'z') then
        Result[1] := char(Ord(Result[1]) - 32); // делаем из маленькой буквы большую 
    end;
  end;
...
begin
    ...
    try
      ...
      if AnyCaseFirst then begin
        // kudin: 17.04.2011 Если не нашли, то пробуем поискать тоже самое, но с маленькой буквы
        Result := TryFind(Low1Ch(aName));
        if Result >= 0 then Exit;
        // kudin: 05.10.2011 Если не нашли, то пробуем поискать тоже самое, но с большой буквы
...
type
    ...
    fNewNames: TXmlNameTable;
  	ToWriteInEmptyXMLDocumentProc: TGetBodyCallback;//Это надо заполнить, если надо вставить внуть тела документа "под-XML"
    EmptyXMLDocumentElementToWriteIn: tXMLString;//Куда надо вставить ToWriteInEmptyXMLDocumentProc
...
begin
    ...
    while str <> '' do begin
      ...
      if Result = Nil then
        Exit;//для однотипности с простым SelectSingleNode
    end;
	end
end;
...
begin
  // kudin: 21.04.2011 Не используем дефолтовую таблицу имен.
  // Связано с тем, что мы игнорируем размер буквы при поиске атрибута по имени,
  // а в результирующий xml пишем в правильном регистре
